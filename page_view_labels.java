...
public class page_view_labels {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        if (Genre_Input.equals("ALL")) {
            String q = "select LABEL_NAME from RECORD_LABELS";
            builder.append("<br><br> DESIRED RECORD LABELS:" + myDB.query(q));
            System.out.println(builder.toString());
            return;
        }

        // Validate user input
        String q1 = "SELECT EXISTS(SELECT * from RECORD_LABELS WHERE DESIRED_GENRE='" + Genre_Input + "') as RESULT";
        ...
        if (result1 == 0) {
            System.out.println(
                    "<br> No Label is under that genre or you have inputted an invalid Genre name, grab one from the above table!");
            return;
        }

        // If valid, select * from ARTISTS where GENRE = 'Genre_Input'
        String q = "select * from RECORD_LABELS where DESIRED_GENRE = '" + Genre_Input + "'";
        builder.append("<br><br> DESIRED RECORD LABELS:" + myDB.query(q));
        System.out.println(builder.toString());
